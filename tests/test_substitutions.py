...
class SubstitutionTests(unittest.TestCase):
    ...
    def test_one_minute_blocks_autosub(self) -> None:
        ...
        multipliers = effective_multipliers(manager(), self.players, values, self.complete)
    ...
    def test_triple_captain_and_bench_boost(self) -> None:
        values = scores()
        triple = effective_multipliers(manager(chip="3xc"), self.players, values, self.complete)
        boosted = effective_multipliers(manager(chip="bboost"), self.players, values, self.complete)
    ...
    def test_transfer_cost_is_subtracted(self) -> None:
        ...
        hit = score_manager(manager(transfer_cost=4), self.players, values, self.complete)
